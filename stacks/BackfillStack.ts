...
export function BackfillStack({ stack }: StackContext) {
  ...
  list2.slice(0, 12)
    .map((dataUrl, index) => {
      const taskDefinitionJob = new ecs.Ec2TaskDefinition(stack, getCdkNames(`task-jobs-list-update-${index}`, stack.stage), {
        networkMode: ecs.NetworkMode.HOST
      })
      tasks.push(taskDefinitionJob)
      const containerName = getCdkNames(`job-container-list-update-${index}`, stack.stage)
      taskDefinitionJob.addContainer(containerName, {
        containerName,
  ...
  // Instantiate an Amazon ECS Service
  tasks.map((taskDefinition, index) => {
    const serviceNameJobs = getCdkNames(`service-update-list-${index}`, stack.stage)
    new ecs.Ec2Service(stack, serviceNameJobs, {
      serviceName: serviceNameJobs,
      cluster,
      taskDefinition,
